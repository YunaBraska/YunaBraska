...
class MyTestCase(unittest.TestCase):
    ...
    def test_invalid_semver(self, mock_print):
        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase=None, output='result', version='1.2.3@rc.4+m5'))
    ...
    def test_valid_semver_with_v(self, mock_print):
        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase=None, output='result', version='v1.2.3-rc.4+m5'))
        mock_print.assert_called_with('1.2.3-rc.4+m5')
        self.assertEqual(cli.exception.code, 0)
    ...
    def test_increase(self, mock_print):
        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='major', output='result', version='1.2.3-rc.4+m5'))
        mock_print.assert_called_with("2.0.0")
        self.assertEqual(cli.exception.code, 0)

        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='minor', output='result', version='1.2.3-rc.4+m5'))
        mock_print.assert_called_with("1.3.0")
        self.assertEqual(cli.exception.code, 0)

        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='patch', output='result', version='1.2.3-rc.4+m5'))
        mock_print.assert_called_with("1.2.4")
        self.assertEqual(cli.exception.code, 0)

        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='rc', output='result', version='1.2.3-rc.4+m5'))
        mock_print.assert_called_with("1.2.3-rc.5")
        self.assertEqual(cli.exception.code, 0)

        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='meta', output='result', version='1.2.3-rc.4+m5'))
    ...
    def test_increase_special_cases(self, mock_print):
        ...
        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='meta', output='result', version='1.2.3-rc.4'))
        ...
        with self.assertRaises(SystemExit) as cli:
            semver.run(argparse.Namespace(increase='rc', output='result', version='1.2.3-rc.4'))
        mock_print.assert_called_with("1.2.3-rc.5")
        self.assertEqual(cli.exception.code, 0)
